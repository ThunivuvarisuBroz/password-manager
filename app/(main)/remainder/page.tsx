...
import { AuthHead } from "@/lib/AuthContext";
...
export default function ReminderForm() {
  ...
  const { isLogin, tokenValue } = useContext(AuthHead);

  const dropdownRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    console.log("tval" + isLogin);
    console.log("tval" + tokenValue);
  ...
  const handleCreateReminder = async () => {
    ...
    console.log("Combined Time:", `${selectedHour}:${selectedMinute} ${period}`);
    const checkToken = Checklogin(tokenValue);
    console.log(checkToken);

    if (isLogin && checkToken === "proceed") {
      const remainderApi = await fetch("/api/remainder", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          Rdate: date,
          Rhour: selectedHour,
          Rminute: selectedMinute,
          Rperiod: period,
        }),
      });

      // if (!remainderApi.ok) {
      //   const errorText = await remainderApi.text();
      //   toast.error(errorText || "Failed to create reminder");
      //   return;
      // }

      const responseApi = await remainderApi.json();
      console.log(responseApi);
      // toast.success("Reminder created successfully");
    } else {
      toast.error(checkToken);
    }
  };
                ...
                <div className="absolute right-0 mt-2 p-4 bg-white border border-slate-200 rounded-2xl shadow-2xl z-50 w-64 animate-in fade-in slide-in-from-top-2 duration-150">
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <div className="relative w-40 h-40 rounded-full border border-slate-100 flex items-center justify-center bg-slate-50">
                      ...
                      {hours.map((hr, index) => {
                        const angle =
                          (index * 30 * Math.PI) / 180 - Math.PI / 2;
